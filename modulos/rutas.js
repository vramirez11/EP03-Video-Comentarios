...
var registroPost = function(req, res, next)
{
	...
	var sql = "select count(*) as numero from users where usuario = '"+(data.username)+"' or email = '"+(data.correo)+"'"; 
	//CREAR SENTENCIA SQL EN ESTA VARIABLE...
	...
	db.queryMysql(sql, function(err, response)
	{
		...
		else
		{
            ...
            sql = "insert into users (nombre, usuario, clave, email) values ('"+(data.nombre)+ 
            	  "','"+(data.username) + "','" + (password) + "','" + 
            	  (data.correo)+"')";//VARIABLE QUE GUARDARÁ LA SENTENCIA SQL...
			console.log(sql);
			db.queryMysql(sql, function(err, response)
			{
